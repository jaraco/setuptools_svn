...
def _run_command(args, stdout=_PIPE, stderr=_PIPE, encoding=None, stream=0):
    ...
    try:
        proc = _Popen(
            args, stdout=stdout, stderr=stderr, shell=(sys.platform == 'win32')
        )
...
def _get_entry_schedule(entry):
    ...
    return "".join(
        [t.nodeValue for t in schedule.childNodes if t.nodeType == t.TEXT_NODE]
    )
...
def _get_target_property(target):
    ...
    return "".join(
        [t.nodeValue for t in property_text.childNodes if t.nodeType == t.TEXT_NODE]
    )
    ...
    for event, node in doc:
        if event == 'START_ELEMENT' and node.nodeName == 'entry':
            ...
            if not _get_entry_schedule(node).startswith('delete'):
                entries.append((node.getAttribute('path'), node.getAttribute('kind')))
...
class SvnInfo(object):
    ...
    def get_svn_version():
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            code, data = _run_command(
                ['svn', '--config-dir', tempdir, '--version', '--quiet']
            )
    ...
    def load(cls, dirname=''):
        ...
        # Real queries in real svn repos with be concerned with it creation
        with tempfile.TemporaryDirectory() as tempdir:
            code, data = _run_command(['svn', '--config-dir', tempdir, 'info', normdir])
        ...
        is_svn_wd = not code or os.path.isfile(os.path.join(svn_dir, 'entries'))
        ...
        if code or not base_svn_version or base_svn_version < (1, 3):
            warnings.warn(
                ("No SVN 1.3+ command found: falling back " "on pre 1.7 .svn parsing"),
                DeprecationWarning,
            )
...
class Svn13Info(SvnInfo):
    def get_entries(self):
        code, data = _run_command(
            ['svn', 'info', '-R', '--xml', self.path], encoding="utf-8"
        )
...
class SvnFileInfo(SvnInfo):
    ...
    def get_externals(self):
        prop_files = [['.svn', 'dir-prop-base'], ['.svn', 'dir-props']]
        externals = []
...
class SVNEntriesFileText(SVNEntriesFile):
    ...
    def parse_revision_numbers(self):
        ...
        rev_numbers = [
            ...
            if (len(section) > revision_line_number and section[revision_line_number])
        ]
        return rev_numbers
...
class SVNEntriesFileXML(SVNEntriesFile):
    ...
    def parse_revision_numbers(self):
        ...
        return [int(m.group(1)) for m in revre.finditer(self.data)]

    def get_undeleted_records(self):
        entries_pattern = re.compile(r'name="([^"]+)"(?![^>]+deleted="true")', re.I)
        results = [
            unescape(match.group(1)) for match in entries_pattern.finditer(self.data)
        ]
        return results
